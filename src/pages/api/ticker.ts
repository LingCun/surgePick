  ...
  return new Response(
    JSON.stringify({
      ...
      _debug: {
        ...
        prices_first: prices[0]?.date,
        prices_last: prices[prices.length - 1]?.date,
        regime_total: regimeRows.length,
        regime_first: regimeRows[0]?.date,
        regime_last: regimeRows[regimeRows.length - 1]?.date,
        today,
      },
      history: recent,
      forecast,
